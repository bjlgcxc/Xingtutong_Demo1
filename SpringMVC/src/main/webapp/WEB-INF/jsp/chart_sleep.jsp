...
function toDayView(startTime,endTime){
	 $("#change").text('>>多日数据视图');
	 $.ajax({
     	url:"sleep/" + <%=deviceId%> + "/getSleepInfo?duration=1&startTime=" + startTime + "&endTime="+endTime,
     	type:"get",
		dataType:'json',
...
function toDaysView(startTime,endTime){
	$("#change").text('>>单日数据视图');
	$.ajax({
     	url:"sleep/" + <%=deviceId%> + "/getSleepInfo?duration=2&startTime=" + startTime + "&endTime="+endTime,
     	type:"get",
		dataType:'json',
		contentType: 'application/json;charset=utf-8',
		success:function(data){ 
